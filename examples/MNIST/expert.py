...
def main():
    ...
    X = tf.placeholder(tf.float32, [None, 28, 28, 1])   # input image == 28x28 pixels
    ...
    W1 = tf.Variable(tf.truncated_normal([5, 5, 1, 32], stddev=0.01))
    ...
    W2 = tf.Variable(tf.truncated_normal([5, 5, 32, 64], stddev=0.01))
    ...
    W3 = tf.Variable(tf.truncated_normal([7 * 7 * 64, 1024], stddev=0.01))
    ...
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=pred_Y, logits=Y))
    ...
    with tf.Session() as sess:
        ...
        for i in range(20000):
            ...
            if i % 100 == 0:
                # evaluate
                accuracy_val = accuracy.eval(feed_dict={X: batch_xs,
                                                        Y: batch_ys,
                                                        keep_prob: 1.0})
